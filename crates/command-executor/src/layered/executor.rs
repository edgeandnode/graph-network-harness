...
use super::{ExecutionLayer, ExecutionContext};
use crate::{Command, launcher::Launcher, target::Target, error::Result, event::ProcessEvent};
...
impl<L: Launcher> LayeredExecutor<L> {
    ...
    pub async fn execute(&self, command: Command, target: &L::Target) -> Result<(EventStream, L::Handle)> {
        // Apply all layers to transform the command
        let final_command = self.layers.iter()
            .try_fold(command, |cmd, layer| layer.wrap_command(cmd, &self.context))?;
...
impl LayeredExecutor<crate::backends::LocalLauncher> {
    ...
    pub async fn execute_command(&self, command: Command) -> Result<(EventStream, crate::backends::LocalProcessHandle)> {
...
mod tests {
    ...
    use crate::layered::{SshLayer, DockerLayer, LocalLayer};
    ...
    fn test_context_building() {
        ...
        assert_eq!(executor.context.env.get("TEST_VAR"), Some(&"test_value".to_string()));
    ...
    fn test_complex_docker_configuration() {
        ...
        assert_eq!(executor.context.env.get("APP_ENV"), Some(&"production".to_string()));
        assert_eq!(executor.context.working_dir, Some("/tmp".into()));
    }
    ...
    fn test_execution_context_propagation() {
        ...
        assert_eq!(executor.context.env.get("TEST_VAR1"), Some(&"value1".to_string()));
        assert_eq!(executor.context.env.get("RUNTIME_VAR"), Some(&"runtime_value".to_string()));
        assert_eq!(executor.context.working_dir, Some("/custom/workdir".into()));
        assert_eq!(executor.context.metadata.get("deployment"), Some(&"staging".to_string()));
    }
