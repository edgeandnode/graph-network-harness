...
#[cfg(test)]
mod tests {
    use super::super::{LayeredExecutor, SshLayer, DockerLayer, LocalLayer, ExecutionContext};
    ...
    fn test_command_transformation_ssh_only() {
        ...
        let args: Vec<&str> = transformed.get_args().iter().map(|s| s.to_str().unwrap()).collect();
    ...
    fn test_multi_hop_ssh_transformation() {
        ...
        let args: Vec<&str> = final_command.get_args().iter().map(|s| s.to_str().unwrap()).collect();
        assert!(args.contains(&"user@target-host"));
        
        // The command string should contain the nested SSH call
        let command_arg = args.iter().find(|&&arg| arg.contains("ssh user@jump-host")).unwrap();
    ...
    fn test_complex_three_layer_transformation() {
        ...
        let args: Vec<&str> = current_command.get_args().iter().map(|s| s.to_str().unwrap()).collect();
        assert!(args.contains(&"exec"));
        assert!(args.contains(&"database-container"));
        
        // The docker command should contain nested SSH calls
        let nested_command = args.iter()
    ...
    fn test_context_environment_propagation() {
        ...
        assert_eq!(executor.context().env.get("GLOBAL_VAR"), Some(&"global_value".to_string()));
    ...
    fn test_docker_layer_specific_environment() {
        ...
        let has_api_key = args.windows(2).any(|pair| 
            pair[0] == "-e" && pair[1] == "API_KEY=secret123"
        );
        let has_debug = args.windows(2).any(|pair| 
            pair[0] == "-e" && pair[1] == "DEBUG=true"
        );
        assert!(has_api_key, "Should have API_KEY environment variable from layer");
        assert!(has_debug, "Should have DEBUG environment variable from layer");
    ...
    fn test_layer_specific_vs_global_environment_separation() {
        ...
        let ssh_layer = SshLayer::new("user@remote")
            .with_env("SSH_SPECIFIC", "ssh_value");
        
        let docker_layer = DockerLayer::new("container")
            .with_env("DOCKER_SPECIFIC", "docker_value");
        ...
        let has_docker_env = args.windows(2).any(|pair| 
            pair[0] == "-e" && pair[1] == "DOCKER_SPECIFIC=docker_value"
        );
        assert!(has_docker_env, "Docker layer should have its specific environment");
        
        // Should NOT have global environment in Docker layer
        let has_global_env = args.windows(2).any(|pair| 
            pair[0] == "-e" && pair[1] == "GLOBAL_VAR=global_value"
        );
        assert!(!has_global_env, "Docker layer should NOT have global environment");
    ...
    fn test_complex_multi_layer_with_different_environments() {
        ...
        let has_app_env = args.windows(2).any(|pair| 
            pair[0] == "-e" && pair[1] == "APP_ENV=production"
        );
        let has_db_url = args.windows(2).any(|pair| 
            pair[0] == "-e" && pair[1] == "DATABASE_URL=postgres://prod-db:5432/app"
        );
        assert!(has_app_env && has_db_url, "Docker layer should have its environment");
        
        // Verify nested SSH command with its environment
        let nested_ssh = args.iter()
    ...
    fn test_ssh_credential_forwarding_options() {
        ...
        let executor = LayeredExecutor::new(LocalLauncher)
            .with_layer(secure_ssh);
        
        let mut command = Command::new("sudo");
        command.arg("-A").arg("systemctl").arg("status").arg("secure-service");
    ...
    fn test_ssh_forwarding_with_docker_combination() {
        ...
        command.arg("clone").arg("git@github.com:company/private-repo.git");
        ...
        let has_build_env = args.windows(2).any(|pair| 
            pair[0] == "-e" && pair[1] == "BUILD_ENV=production"
        );
