...
mod tests {
    ...
    async fn test_stdin_handle_basic() {
        ...
        assert!(handle.stdin_mut().is_some(), "Stdin handle should be available");
    ...
    async fn test_stdin_write_to_process() {
        ...
        if let Ok(contents) = std::fs::read_to_string(&temp_file) {
            ...
            assert!(contents.contains("Test line 1"), "File should contain test line 1");
            assert!(contents.contains("Test line 2"), "File should contain test line 2");
        }
