...
impl TestHarness {
    ...
    pub async fn setup() -> Result<Self> {
        ...
        std::fs::create_dir_all(&ssh_keys_dir).context("Failed to create SSH keys directory")?;
    ...
    async fn generate_ssh_keys(&self, ssh_keys_dir: &Path) -> Result<()> {
        ...
        let keygen_cmd = Command::builder("ssh-keygen")
            ...
            .arg("")
            .arg("-C")
            .arg("test@command-executor")
            .build();

        let result = self
            .executor
            .execute(&Target::Command, keygen_cmd)
            .await
    ...
    pub async fn teardown(self) -> Result<()> {
        ...
        let _ = self
            .executor
            .execute(&Target::Command, down_cmd)
            .await
