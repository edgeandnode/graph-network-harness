...
fn test_attach_to_running_service() {
    futures::executor::block_on(async {
        ...
        assert!(result.is_ok(), "Should successfully attach to running service");
...
fn test_service_lifecycle_control() {
    futures::executor::block_on(async {
        ...
        // Test disconnect
        assert!(handle.disconnect().await.is_ok(), "Disconnect should succeed");
    });
}
...
fn test_service_without_reload() {
    futures::executor::block_on(async {
        ...
        // Reload should fail
        assert!(handle.reload().await.is_err(), "Reload should fail when not supported");
    });
}
...
fn test_restart_fallback() {
    futures::executor::block_on(async {
        ...
        assert!(handle.restart().await.is_ok(), "Restart should succeed with fallback");
    });
}
...
fn test_command_failure_handling() {
    futures::executor::block_on(async {
        ...
        // Start should fail
        assert!(handle.start().await.is_err(), "Start should fail with exit code 1");
        
        // Stop should fail
        assert!(handle.stop().await.is_err(), "Stop should fail with exit code 1");
    });
}
...
fn test_log_streaming_cleanup() {
    futures::executor::block_on(async {
        ...
        let service = ManagedService::builder("cleanup-test")
            ...
            .start_command(Command::new("true"))
            .stop_command(Command::new("true"))
            .log_command(Command::builder("sh").arg("-c")
                .arg("for i in 1 2 3; do echo line $i; sleep 0.01; done").build())
            .build()
            .unwrap();
