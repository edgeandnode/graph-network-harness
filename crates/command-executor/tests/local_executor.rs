...
use command_executor::{
    Command, Executor, ManagedProcess, ProcessEvent, ProcessEventType, ProcessHandle, Target,
};
...
fn test_managed_process_target() {
    futures::executor::block_on(async {
        ...
        let cmd = Command::builder("echo").arg("managed process").build();
...
fn test_event_streaming() {
    futures::executor::block_on(async {
        ...
        let stderr_events: Vec<_> = collected
            .iter()
