...
use command_executor::{Command, Executor, ProcessEventType, ProcessHandle};
use command_executor::{SystemdPortable, Target};
...
fn test_real_portablectl_attach_detach() {
    ...
    futures::executor::block_on(async {
        ...
        assert_eq!(
            exit_status.code,
            Some(0),
            "portablectl attach should succeed"
        );
        ...
        assert!(
            output.contains("echo-service"),
            "Service should be listed after attach"
        );
        ...
        assert_eq!(
            exit_status.code,
            Some(0),
            "portablectl detach should succeed"
        );
    });
}
...
fn test_portable_service_logs() {
    ...
    futures::executor::block_on(async {
        ...
        assert!(
            log_output.contains("Echo service"),
