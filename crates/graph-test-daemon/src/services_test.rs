...
mod tests {
    ...
    fn test_service_stack_registration() {
        ...
        stack.register("graph-node-1".to_string(), graph_node).unwrap();
    ...
    fn test_complete_graph_stack() {
        smol::block_on(async {
            ...
            stack.register("graph-node-1".to_string(), 
                GraphNodeService::new("localhost".to_string())).unwrap();
            stack.register("anvil-1".to_string(), 
                AnvilService::new(31337, 8545)).unwrap();
            stack.register("postgres-1".to_string(), 
                PostgresService::new("graph-node".to_string(), 5432)).unwrap();
            stack.register("ipfs-1".to_string(), 
                IpfsService::new(5001, 8080)).unwrap();
            ...
            let postgres_result = stack.dispatch(
                "postgres-1", 
                "default", 
            ...
            let ipfs_result = stack.dispatch(
                "ipfs-1",
                "default",
