...
fn test_all_configs_load_successfully() {
    ...
    assert!(!yaml_files.is_empty(), "Should have at least one YAML config file");
    
    for yaml_path in yaml_files {
        let file_name = yaml_path.file_name()
            .and_then(|s| s.to_str())
            .unwrap_or("unknown");
            ...
            Ok(config) => {
                ...
                for (service_name, service) in &config.services {
                    ...
                    assert!(!service.service_type.is_empty(), 
                           "Service {} should have a service_type", service_name);
                    ...
                    if let service_orchestration::ServiceTarget::Process { command, .. } = 
                        ...
                        assert!(!cmd_parts.is_empty(), 
                               "Service {} should have a valid command", service_name);
                    }
                }
                ...
                for (task_name, task) in &config.tasks {
                    ...
                    assert!(!task.task_type.is_empty(), 
                           "Task {} should have a task_type", task_name);
                }
            }
            Err(e) => {
...
fn test_graph_stack_config_specifics() {
    ...
    let content = fs::read_to_string(&config_path)
        .expect("Failed to read graph-stack.yaml");
        
    let config: StackConfig = serde_yaml::from_str(&content)
        .expect("Failed to parse graph-stack.yaml");
    
    // Verify expected services exist
    assert!(config.services.contains_key("postgres"), "Should have postgres service");
    assert!(config.services.contains_key("ipfs"), "Should have ipfs service");
    assert!(config.services.contains_key("anvil"), "Should have anvil service");
    assert!(config.services.contains_key("graph-node"), "Should have graph-node service");
    ...
    if let service_orchestration::ServiceTarget::Process { command, .. } = 
        ...
        match command {
            service_orchestration::ProcessCommand::Template { params, command_template, .. } => {
                // Check required params exist
                assert!(params.contains_key("binary"), "Anvil should have binary param");
                assert!(params.contains_key("port"), "Anvil should have port param");
                assert!(params.contains_key("chain_id"), "Anvil should have chain_id param");
                
                // Check template contains expected placeholders
                assert!(command_template.contains("{binary}"), "Template should reference binary");
                assert!(command_template.contains("{port}"), "Template should reference port");
                assert!(command_template.contains("{chain_id}"), "Template should reference chain_id");
            }
    ...
    let dep_names: Vec<String> = deps.iter().map(|d| match d {
        service_orchestration::Dependency::Service { service } => service.clone(),
        service_orchestration::Dependency::Task { task } => task.clone(),
    }).collect();
    
    assert!(dep_names.contains(&"postgres".to_string()), "Should depend on postgres");
    assert!(dep_names.contains(&"ipfs".to_string()), "Should depend on ipfs");
    assert!(dep_names.contains(&"anvil".to_string()), "Should depend on anvil");
}
...
fn test_config_parameter_substitution() {
    ...
    if let service_orchestration::ServiceTarget::Docker { params, env, .. } = 
        ...
        assert!(params.contains_key("user"), "Postgres should have user param");
        assert!(params.contains_key("password"), "Postgres should have password param");
        assert!(params.contains_key("database"), "Postgres should have database param");
        
        // Check env uses param substitution
        assert!(env.get("POSTGRES_USER").unwrap().contains("{user}"), 
               "POSTGRES_USER should reference user param");
        assert!(env.get("POSTGRES_PASSWORD").unwrap().contains("{password}"), 
               "POSTGRES_PASSWORD should reference password param");
        assert!(env.get("POSTGRES_DB").unwrap().contains("{database}"), 
