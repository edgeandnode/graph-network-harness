...
impl YamlTask {
    ...
    async fn validate_idempotent(&self) -> Result<bool, Error> {
        if let Some(validation_cmd) = &self.config.validation_command {
            debug!("Running validation command for {}: {}", self.name, validation_cmd);
            ...
            match self.execute_cmd(cmd).await {
                Ok(mut handle) => {
                    let exit = handle.wait().await
                        .map_err(|e| Error::action(format!("Failed to wait for validation command: {}", e)))?;
    ...
    async fn execute_cmd(&self, cmd: Command) -> Result<Box<dyn ProcessHandle>, Error> {
        match &self.target {
            ServiceTarget::Process { .. } => {
                ...
                let (_events, handle) = launcher.launch(&target, cmd).await
                    .map_err(|e| Error::action(format!("Failed to launch command: {}", e)))?;
                Ok(Box::new(handle))
            }
            ServiceTarget::Docker { .. } => {
                // Docker execution
                Err(Error::validation("Docker launcher not yet implemented for YAML tasks"))
            }
            ServiceTarget::Layered { .. } => {
                ...
                Err(Error::validation("Layered execution not yet implemented for YAML tasks"))
            }
            _ => {
                Err(Error::validation("Unsupported target type for YAML task"))
            }
        }
    }
    ...
    async fn execute_command(&self) -> Result<(), Error> {
        info!("Executing YAML task '{}': {}", self.name, self.config.command);
    ...
    async fn execute(&self) -> Result<Receiver<Self::State>, Error> {
        ...
        smol::spawn(async move {
            ...
            match task.validate_idempotent().await {
                Ok(true) => {
                    info!("Task '{}' already completed (validation passed), skipping", name);
    ...
    fn from_config(config: &TaskConfig) -> Result<Self, Error> {
        ...
        let command = match &config.target {
            ServiceTarget::Process { command, .. } => {
                ...
                match command {
                    service_orchestration::ProcessCommand::Template { command_template, params, .. } => {
                    ...
                    service_orchestration::ProcessCommand::Legacy { command } => {
                        command.clone()
                    }
                }
            }
            ServiceTarget::Layered { command_template: Some(template), params, .. } => {
                // Substitute params in command template
            ...
            _ => {
                return Err(Error::validation("YAML task requires command or command_template"));
            }
        };
