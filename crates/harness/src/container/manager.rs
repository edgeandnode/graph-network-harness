...
impl DindManager {
    ...
    fn get_log_path(&self, name: &str) -> PathBuf {
        self.config
            .log_dir
            .join(format!("{}_{}.log", self.session_id, name))
    }
    ...
    async fn find_container(&self) -> Result<Option<ContainerSummary>> {
        ...
        let container = containers.into_iter().find(|c| {
            if let Some(names) = &c.names {
                names
                    .iter()
                    .any(|name| name.contains("integration-tests-dind"))
            } else {
                false
            }
        });
    ...
    async fn start_with_compose(&mut self) -> Result<()> {
        info!(
            "Starting docker-compose from directory: {:?}",
        ...
        if !self.config.docker_test_env_path.exists() {
            anyhow::bail!(
                "docker-test-env path does not exist: {:?}",
                self.config.docker_test_env_path
            );
        }
        ...
        info!(
            "Starting container with docker-compose, logging to: {:?}",
            log_path
        );
    ...
    async fn wait_for_docker_ready(&self) -> Result<()> {
        let _container_id = self.container_id.as_ref().context("No container ID set")?;
    ...
    pub async fn exec_simple(&self, cmd: Vec<&str>) -> Result<String> {
        let container_id = self.container_id.as_ref().context("No container ID set")?;
    ...
    pub async fn exec_in_container(
        ...
        info!(
            "Executing in container: {:?}, logging to: {:?}",
            cmd, log_path
        );
        ...
        let mut exec_options = CreateExecOptions {
            ...
            tty: Some(true),
            env: Some(vec!["CARGO_TERM_COLOR=never".to_string()]),
            ..Default::default()
        };
    ...
    pub async fn build_in_container(&self) -> Result<()> {
        ...
        let exit_code = self
            .exec_in_container(
                vec!["cargo", "build", "--color=never", "--bin", "stack-runner"],
                Some("/workspace"),
            )
            .await?;
    ...
    pub async fn sync_images(&self) -> Result<crate::container::image_sync::ImageSyncResult> {
        ...
        let image_sync =
            ImageSync::new(self.docker.clone(), container_id.clone(), compose_file_path);
    ...
    pub async fn build_host_images(&self) -> Result<()> {
        ...
        info!(
            "Building images on host using build-with-overrides.sh, logging to: {:?}",
            log_path
        );
        ...
        let mut child = Command::new("bash")
            ...
            .env(
                "INDEXER_AGENT_SOURCE_ROOT",
                indexer_source.to_string_lossy().to_string(),
            )
            .stdout(std::process::Stdio::piped())
            .stderr(std::process::Stdio::piped())
            .spawn()
    ...
    pub fn print_log_summary(&self) {
        info!(
            "Container session logs saved to: {}",
        ...
        if self
            .config
            .log_dir
            .join(format!("{}_build-images.log", self.session_id))
            .exists()
        {
...
mod tests {
    ...
    fn test_session_id_format() {
        ...
        let parsed = DateTime::parse_from_str(
            &format!("{} +0000", session_id.replace('_', " ")),
            "%Y-%m-%d %H-%M-%S %z",
        );
        assert!(parsed.is_ok());
    }
