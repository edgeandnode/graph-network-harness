...
async fn test_daemon_websocket_connection() -> Result<()> {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::task::spawn_blocking(move || {
            ctx_clone.run_cli_command(&["status"])
        });
        handles.push(handle);
    }
...
async fn test_daemon_connection_failure_handling() -> Result<()> {
    ...
    let output = ctx.run_cli_command_with_env(
        &["status"],
        &[("HARNESS_DAEMON_PORT", &port.to_string())]
    )?;
...
async fn test_concurrent_modifications() -> Result<()> {
    ...
    let stop_handle = tokio::task::spawn_blocking(move || {
        ctx_ref.run_cli_command(&["stop", "echo-service"])
    });
...
async fn test_daemon_graceful_shutdown() -> Result<()> {
    ...
    output
        .assert_success()
        .assert_contains("Daemon stopped");
    ...
    output
        .assert_failure()
        .assert_contains("Failed to connect");
...
async fn test_daemon_state_persistence() -> Result<()> {
    ...
    let mut ctx = CliTestContext::with_daemon_args(&[
        "--state-dir", state_path.to_str().unwrap()
    ]).await?;
