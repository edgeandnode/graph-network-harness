...
"#;
    ...
    let running_count = status_lines.iter()
...
"#)?;
    ...
    let output = ctx.run_cli_command(&[
        "start", 
        "-f", staging_config.to_str().unwrap(),
        "--all"
    ])?;
    output.assert_success();
