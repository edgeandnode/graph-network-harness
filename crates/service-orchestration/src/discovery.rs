...
use service_registry::{Registry, ServiceEntry, ServiceState as RegistryServiceState, Endpoint};
...
impl ServiceDiscovery {
    ...
    pub async fn discover_by_type(&self, service_type: &str) -> Result<Vec<ServiceEndpoint>, Error> {
        ...
        info!("Discovered {} endpoints for service type '{}'", endpoints.len(), service_type);
        Ok(endpoints)
    }
    ...
    pub async fn wait_for_service(&self, name: &str, max_attempts: u32) -> Result<ServiceEntry, Error> {
        ...
        for attempt in 1..=max_attempts {
            if let Some(service) = self.discover_service(name).await? {
                ...
                debug!("Service '{}' found but not running (state: {:?})", name, service.state);
            }
            
            if attempt < max_attempts {
                debug!("Attempt {}/{} - service '{}' not ready, waiting...", attempt, max_attempts, name);
                #[cfg(feature = "smol")]
    ...
    pub async fn build_service_config(
                    ...
                    if let Some(dep_service) = self.discover_service(service).await? {
                        ...
                        for (idx, endpoint) in dep_service.endpoints.iter().enumerate() {
                            ...
                            let value = match endpoint.protocol {
                                service_registry::Protocol::Http => format!("http://{}", endpoint.address),
                                service_registry::Protocol::Https => format!("https://{}", endpoint.address),
                                service_registry::Protocol::Grpc => format!("grpc://{}", endpoint.address),
                                service_registry::Protocol::Tcp => endpoint.address.to_string(),
                                service_registry::Protocol::WebSocket => format!("ws://{}", endpoint.address),
                                service_registry::Protocol::Custom(ref proto) => format!("{}://{}", proto, endpoint.address),
                            };
                        ...
                        if let Some(primary) = dep_service.endpoints.first() {
                            config.insert(format!("{}_HOST", service.to_uppercase()), primary.address.ip().to_string());
                            config.insert(format!("{}_PORT", service.to_uppercase()), primary.address.port().to_string());
                        }
                    } else {
        ...
        info!("Built configuration with {} entries for service", config.len());
        Ok(config)
    }
}
...
mod tests {
    use super::*;
    use service_registry::{Registry, ServiceEntry, ExecutionInfo, Location};
    use chrono::Utc;
    ...
    async fn test_build_service_config() {
        ...
        let config = discovery.build_service_config(&service_config).await.unwrap();
