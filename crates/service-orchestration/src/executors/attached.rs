...
impl AttachedService for SystemdAttachedExecutor {
    async fn attach(&self, config: ServiceConfig) -> std::result::Result<RunningService, Error> {
        ...
        self.attached_services
            .lock()
            .await
            .insert(config.name.clone(), Box::new(handle));
    ...
    async fn attach(&self, config: ServiceConfig) -> std::result::Result<RunningService, Error> {
        match &config.target {
            ServiceTarget::Docker { .. } => {
                let env = config.target.env();
                let container_name = env.get("CONTAINER_NAME").ok_or_else(|| {
                    crate::Error::Config("CONTAINER_NAME not specified for attachment".into())
                })?;
            ...
            _ => Err(crate::Error::Config(
                "Docker attacher requires Docker target".into(),
            )),
        }
    }
...
impl LocalProcessAttachedExecutor {
    ...
    pub fn new() -> Self {
        Self {
            executor: command_executor::Executor::new(
                "local-process-attacher".to_string(),
                LocalLauncher,
            ),
    ...
    async fn find_pid_by_name(&self, process_name: &str) -> std::result::Result<u32, Error> {
        ...
        let result = self
            .executor
            .execute(&command_executor::target::Target::Command, cmd)
            .await?;
    ...
    async fn attach(&self, config: ServiceConfig) -> std::result::Result<RunningService, Error> {
        ...
        let pid = if let Some(pid_str) = env.get("PID") {
            ...
            return Err(Error::Config(
                "Either PID or PROCESS_NAME must be specified".to_string(),
            ));
        };
