    ...
    async fn stop(&self, service: &RunningService) -> std::result::Result<(), Error> {
        ...
        if let Some(mut process_info) = process_info {
            ...
            match process_info.handle.terminate().await {
                ...
                Err(e) => {
                    warn!(
                        "Failed to terminate service {}: {}, trying kill",
                        service.name, e
                    );
                    // Try force kill
            ...
            warn!(
                "No process handle found for service {}, using kill command",
                service.name
            );
        ...
        while start.elapsed() < timeout {
            match futures::stream::StreamExt::next(&mut event_stream).await {
                Some(event) => {
                    if matches!(
                        event.event_type,
                        command_executor::event::ProcessEventType::Stdout
                            | command_executor::event::ProcessEventType::Stderr
                    ) {
                        events.push(event);
                    }
                }
