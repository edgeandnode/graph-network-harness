...
use crate::{
    Error, health::{HealthMonitor, HealthStatus},
    context::OrchestrationContext, config::ServiceConfig,
};
...
impl HealthMonitoringManager {
    ...
    pub fn start_monitoring(
        ...
        if let Some(health_check) = service_config.health_check {
            ...
            info!("Health monitoring started for service '{}' with interval {:?}", service_name, interval);
        } else {
...
async fn monitor_service_health(
    ...
    info!("Health monitoring task started for service '{}'", service_name);
    ...
    let Some(config) = health_config else {
        warn!("No health check config found for service '{}'", service_name);
        return;
    };
...
async fn trigger_service_recovery(service_name: &str, context: &Arc<OrchestrationContext>) {
    ...
    if let Some(entry) = service_entry {
        ...
        warn!("Service '{}' not found in registry for recovery", service_name);
    }
}
...
impl HealthMonitoringExt for OrchestrationContext {
    fn health_monitoring(&self) -> Arc<HealthMonitoringManager> {
        HEALTH_MONITORING_MANAGER.get_or_init(|| {
            Arc::new(HealthMonitoringManager::new(Arc::new(self.clone())))
        }).clone()
    }
}
...
mod tests {
    ...
    async fn test_health_monitoring_manager() {
        ...
        manager.start_monitoring("test-service".to_string(), service_config).unwrap();
