...
mod tests {
    ...
    fn test_ip_allocator_creation() {
        ...
        assert_eq!(allocator.get_service_by_ip(&"10.42.0.1".parse().unwrap()), Some("gateway"));
    }
    ...
    fn test_specific_allocation() {
        ...
        allocator.allocate_specific("special-service", specific_ip).unwrap();
        
        // Verify allocation
        assert_eq!(allocator.get_allocation("special-service"), Some(specific_ip));
        assert_eq!(allocator.get_service_by_ip(&specific_ip), Some("special-service"));
