...
impl TestHarness {
    ...
    pub async fn deploy_service(&self, service: ServiceEntry) -> anyhow::Result<ServiceDeployment<'_>> {
    ...
    pub async fn execute_and_track(&self, service_name: &str, cmd: Command, target: &Target) -> anyhow::Result<()> {
        // Update to starting state
        self.registry.update_state(service_name, service_registry::models::ServiceState::Starting).await?;
        ...
        match &result {
            Ok(output) if output.success() => {
                self.registry.update_state(service_name, service_registry::models::ServiceState::Running).await?;
            }
            Ok(_) | Err(_) => {
                self.registry.update_state(service_name, service_registry::models::ServiceState::Failed).await?;
            }
        }
        
        result.map_err(|e| anyhow::anyhow!("Command execution failed: {}", e)).map(|_| ())
    }
    ...
    pub async fn wait_for_service_state(
        ...
        anyhow::bail!(
            ...
            service_name, expected_state, timeout
        )
    }
...
impl<'a> ServiceDeployment<'a> {
    ...
    pub async fn stop(&self) -> anyhow::Result<()> {
        self.registry.update_state(&self.name, service_registry::models::ServiceState::Stopping).await?;
        self.registry.update_state(&self.name, service_registry::models::ServiceState::Stopped).await?;
        Ok(())
    }
...
impl MultiNodeTestEnvironment {
    ...
    pub async fn deploy_to_node(&self, node_index: usize, service: ServiceEntry) -> anyhow::Result<ServiceDeployment<'_>> {
