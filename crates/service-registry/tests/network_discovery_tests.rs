...
use command_executor::{Executor, Target, Command, backends::local::LocalLauncher};
use std::path::Path;
...
async fn ensure_dind_container_running() -> anyhow::Result<()> {
    ...
    let docker_cmd = Command::builder("docker")
        .arg("run")
        .arg("-d")
        .arg("--name").arg(DIND_CONTAINER_NAME)
        .arg("--privileged")
        .arg("-e").arg("DOCKER_TLS_CERTDIR=")
        .arg("docker:dind")
        .build();
...
async fn wait_for_docker_daemon_ready(executor: &Executor<LocalLauncher>) -> anyhow::Result<()> {
    ...
    for i in 1..=max_attempts {
        ...
        if i == max_attempts {
            anyhow::bail!("Docker daemon failed to start in container after {} seconds", max_attempts);
        }
...
async fn dind_compose_command(compose_file: &str, project_name: &str, args: Vec<&str>) -> anyhow::Result<()> {
    let launcher = LocalLauncher;
    ...
    let mut cmd = Command::builder("docker")
        ...
        .arg("docker")
        .arg("compose")
        .arg("-f").arg(format!("/compose/{}", compose_file))
...
async fn check_docker() -> bool {
    ...
    executor.execute(&Target::Command, cmd)
        .await
        .map(|r| r.success())
        .unwrap_or(false)
}
...
fn test_local_network_topology() {
    ...
    smol::block_on(async {
        ...
        ensure_dind_container_running().await
        ...
        dind_compose_command("local-only.yml", &project_name, vec!["up", "-d"]).await
        ...
        network_manager.register_service(ServiceNetwork {
        ...
        }).await.expect("Failed to register graph-node");
        ...
        network_manager.register_service(ServiceNetwork {
            ...
            wireguard_public_key: None,
            interfaces: vec!["docker0".to_string()],
        }).await.expect("Failed to register indexer");
        
        // Verify no WireGuard needed
        assert!(!network_manager.requires_wireguard());
        
        // Test IP resolution
        let ip = network_manager.resolve_service_ip("graph-node", "indexer")
            .expect("Failed to resolve IP");
        assert_eq!(ip.to_string(), "172.100.0.30");
        
        // Test environment generation
        let env = network_manager.generate_environment("graph-node")
        ...
        dind_compose_command("local-only.yml", &project_name, vec!["down", "-v"]).await
...
fn test_lan_network_topology() {
    ...
    smol::block_on(async {
        ...
        // Start compose stack in DinD container
        dind_compose_command("lan-simple.yml", &project_name, vec!["up", "-d"]).await
            .expect("Failed to start docker-compose in DinD");
...
"#;
        ...
        let config: NetworkConfig = serde_yaml::from_str(config_yaml)
            .expect("Failed to parse network config YAML");
...
"#;
        ...
        let services: Vec<ServiceNetwork> = serde_yaml::from_str(services_yaml)
            .expect("Failed to parse services YAML");
        
        for service in services {
            network_manager.register_service(service).await
        ...
        let ip = network_manager.resolve_service_ip("lan-node-1", "lan-node-2")
            .expect("Failed to resolve IP");
        assert_eq!(ip.to_string(), "192.168.100.30");
        
        // Test harness to LAN resolution
        let ip = network_manager.resolve_service_ip("harness", "lan-node-1")
            .expect("Failed to resolve IP");
        assert_eq!(ip.to_string(), "192.168.100.20");
        
        // Cleanup - stop compose stack in DinD container
        dind_compose_command("lan-simple.yml", &project_name, vec!["down", "-v"]).await
            .expect("Failed to stop docker-compose in DinD");
    });
}
...
fn test_mixed_network_topology() {
    ...
    smol::block_on(async {
        ...
        // Start compose stack in DinD container
        dind_compose_command("mixed-topology.yml", &project_name, vec!["up", "-d"]).await
            .expect("Failed to start docker-compose in DinD");
...
"#;
        ...
        let ip = network_manager.resolve_service_ip("local-service", "remote-service")
            .expect("Failed to resolve IP");
        assert_eq!(ip.to_string(), "10.42.0.10");
        
        // Test resolution from LAN to WireGuard
        let ip = network_manager.resolve_service_ip("lan-service", "remote-service")
            .expect("Failed to resolve IP");
        assert_eq!(ip.to_string(), "10.42.0.10");
        
        // Test local to LAN resolution
        let ip = network_manager.resolve_service_ip("local-service", "lan-service")
            .expect("Failed to resolve IP");
        assert_eq!(ip.to_string(), "192.168.100.20");
        
        // Test environment generation includes correct IPs
        let env = network_manager.generate_environment("local-service")
            .expect("Failed to generate environment");
        assert_eq!(env.get("LAN_SERVICE_ADDR"), Some(&"192.168.100.20".to_string()));
        assert_eq!(env.get("REMOTE_SERVICE_ADDR"), Some(&"10.42.0.10".to_string()));
        
        // Cleanup - stop compose stack in DinD container
        dind_compose_command("mixed-topology.yml", &project_name, vec!["down", "-v"]).await
            .expect("Failed to stop docker-compose in DinD");
    });
}
...
fn test_ip_allocation_with_network_manager() {
    smol::block_on(async {
        ...
        network_manager.register_service(ServiceNetwork {
            ...
            interfaces: vec![],
        }).await.expect("Failed to register service");
