...
use service_registry::models::{ServiceState, EventType};
use command_executor::{Target, Command, target::DockerContainer};
...
integration_test!(
    ...
    features = ["integration-tests"],
    {
        ...
        let deployment = harness.deploy_service(service).await.expect("Failed to deploy service");
        
        // Execute command locally
        let cmd = Command::builder("echo").arg("Local process test").build();
        harness.execute_and_track(&deployment.name, cmd, &Target::Command).await
            .expect("Failed to execute command");
        
        // Wait for service to be running
        harness.wait_for_service_state(&deployment.name, ServiceState::Running, std::time::Duration::from_secs(5)).await
...
integration_test!(
    test_docker_container_variant,
    features = ["integration-tests", "docker-tests"],
    {
        let harness = TestHarness::new().await.expect("Failed to create test harness");
        ...
        let cmd = Command::builder("echo").arg("Docker container test").build();
        
        match harness.execute_and_track(&deployment.name, cmd, &Target::DockerContainer(container)).await {
            Ok(_) => {
...
integration_test!(
    ...
    features = ["integration-tests"],
    {
        ...
        let cmd = Command::builder("systemctl").args(["status", "test-daemon.service"]).build();
        
        // This will likely fail, but we test the flow
        let _ = harness.execute_and_track(&deployment.name, cmd, &Target::Command).await;
        
        // In test environment, systemd services will fail
        let final_state = deployment.state().await.unwrap();
        assert!(matches!(final_state, ServiceState::Failed | ServiceState::Running));
    }
);

/// Test systemd portable execution variant
integration_test!(
    test_systemd_portable_variant,
    features = ["integration-tests"],
    {
        let harness = TestHarness::new().await.expect("Failed to create test harness");
        ...
        let result = harness.execute_and_track(&deployment.name, cmd, &Target::Command).await;
...
integration_test!(
    test_remote_ssh_variant,
    features = ["integration-tests", "ssh-tests"],
    {
        let harness = TestHarness::new().await.expect("Failed to create test harness");
        ...
        let retrieved_service = harness.registry.get(&deployment.name).await
            .expect("Failed to get remote service");
        
        // Verify it's marked as remote
        assert!(matches!(retrieved_service.location, service_registry::Location::Remote { .. }));
    }
);
...
integration_test!(
    ...
    features = ["integration-tests"],
    {
        let env = MultiNodeTestEnvironment::new(3).await.expect("Failed to create multi-node environment");
        
        // Deploy coordinator to node 0
        let coordinator = create_echo_service().expect("Failed to create coordinator");
        let coord_deployment = env.deploy_to_node(0, coordinator).await
        ...
        let worker1_deployment = env.deploy_to_node(1, worker1).await
            .expect("Failed to deploy worker 1");
        let worker2_deployment = env.deploy_to_node(2, worker2).await
            .expect("Failed to deploy worker 2");
        
        // Verify all services are deployed
        let all_services = env.list_all_services().await.expect("Failed to list all services");
...
integration_test!(
    ...
    features = ["integration-tests"],
    {
        ...
        for service in stack.services {
            let _deployment = harness.deploy_service(service).await
        ...
        let service_names: std::collections::HashSet<String> = services.iter()
            .map(|s| s.name.clone())
            .collect();
        ...
        let backend = harness.registry.get("backend").await
            .expect("Failed to get backend service");
        assert!(backend.depends_on.contains(&"database".to_string()));
        
        let frontend = harness.registry.get("frontend").await
...
integration_test!(
    ...
    features = ["integration-tests"],
    {
        ...
        harness.registry.subscribe(client_addr, vec![
        ...
        for state in states {
            let (_old_state, events) = harness.registry.update_state(&deployment.name, state).await
        ...
        // Remove service
        let (_service, events) = harness.registry.deregister(&deployment.name).await
...
integration_test!(
    ...
    features = ["integration-tests"],
    {
        ...
        // First harness instance
        {
            ...
            registry.register(service).await.expect("Failed to register service");
            registry.update_state("echo-service", ServiceState::Starting).await
                .expect("Failed to update state to Starting");
            registry.update_state("echo-service", ServiceState::Running).await
                .expect("Failed to update state to Running");
        } // Registry should persist to file
        
        // Second harness instance - load from file
        {
            let registry = service_registry::Registry::load(&persist_path).await
...
integration_test!(
    ...
    features = ["integration-tests"],
    {
        ...
        harness.registry.update_state(&deployment.name, ServiceState::Starting).await
            .expect("Failed to transition to starting");
        harness.registry.update_state(&deployment.name, ServiceState::Running).await
            .expect("Failed to transition to running");
        
        let result = harness.registry.update_state(&deployment.name, ServiceState::Starting).await;
        ...
        let service = harness.registry.get(&deployment.name).await
