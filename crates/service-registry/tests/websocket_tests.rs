...
use service_registry::{Registry, WsServer, EventType, ServiceState};
use std::sync::Arc;
...
fn test_websocket_basic_operations() {
    smol::block_on(async {
        ...
        let server = WsServer::new(addr, registry).await
...
#[test]
#[cfg(feature = "integration-tests")]
fn test_websocket_service_operations() {
    smol::block_on(async {
        ...
        let server = WsServer::new("127.0.0.1:0", registry).await
            .expect("Failed to create server");
        let server_addr = server.listener.local_addr()
            .expect("Failed to get server address");
        ...
        let service = client.get_service("echo-service").await
            .expect("Failed to get service");
        assert_eq!(service["name"], "echo-service");
        assert_eq!(service["state"], "registered");
        
        // Start the service
        client.start_service("echo-service").await
...
#[test]
#[cfg(feature = "integration-tests")]
fn test_websocket_event_subscriptions() {
    smol::block_on(async {
        let registry = Registry::new();
        ...
        server_registry.update_state("web-service", ServiceState::Starting).await
