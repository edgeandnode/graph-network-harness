...
async fn test_dind_container_lifecycle() -> Result<()> {
    ...
    if !docker_test_env_path.exists() {
        eprintln!("Skipping test: docker-test-env not found at {:?}", docker_test_env_path);
        return Ok(());
    }
    ...
    let exit_code = manager.exec_in_container(vec!["echo", "Hello from DinD"], None).await?;
    ...
    let exit_code = manager.exec_in_container(vec!["docker", "version"], None).await?;
    assert_eq!(exit_code, 0, "Docker should be available inside DinD container");
    ...
    assert!(compose_content.contains("[") && compose_content.contains("]"), "Log should have timestamp brackets");
    
    let echo_content = fs::read_to_string(&echo_log).await?;
    assert!(echo_content.contains("Hello from DinD"), "Echo output should be in log");
    assert!(echo_content.contains("[stdout]"), "Log should indicate stdout");
...
async fn test_container_multiple_commands() -> Result<()> {
    ...
    for (cmd, workdir) in commands {
        ...
        let exit_code = manager.exec_in_container(cmd.clone(), Some(workdir)).await?;
    ...
    assert!(pwd_content.contains("/workspace"), "pwd should show /workspace");
...
async fn test_log_streaming_multiline() -> Result<()> {
    ...
    let exit_code = manager.exec_in_container(
        vec!["sh", "-c", script],
        None
    ).await?;
    ...
    for i in 1..=5 {
        assert!(content.contains(&format!("Line {}", i)), "Line {} should be in log", i);
    }
    ...
    assert!(timestamp_count >= 7, "Should have at least 7 timestamped log entries, found {}", timestamp_count);
    
    // Verify proper timestamp format
    assert!(content.contains("[2025-") || content.contains("[2024-"), "Should have year in timestamp");
...
async fn test_failed_command() -> Result<()> {
    ...
    let exit_code = manager.exec_in_container(
        vec!["sh", "-c", "echo 'About to fail' && exit 42"],
        None
    ).await?;
